...
legendre_0 = 1
legendre_1 = x
legendre_2 = 0.5 * (3 * (x ** 2) - 1)
legendre_3 = 0.5 * (5 * (x ** 3) - 3 * x)

legendre_basis = [legendre_0, legendre_1, legendre_2, legendre_3]


def approximate(polynom, diapason, polynom_degree, basis_list):
    x = sp.symbols('x')

    gram_matrix = np.zeros((polynom_degree - 1, polynom_degree - 1))
    for line_ind, line in enumerate(gram_matrix):
        for row_ind, _ in enumerate(line):
            gram_matrix[line_ind, row_ind] = sp.integrate(basis_list[line_ind] * basis_list[row_ind],
                                                          (x, diapason[0], diapason[1]))

    right_vector = np.zeros((polynom_degree - 1, 1))
    for line_ind, _ in enumerate(right_vector):
        right_vector[line_ind, 0] = sp.integrate(basis_list[line_ind] * polynom, (x, diapason[0], diapason[1]))

    coefficients_vector = np.linalg.solve(gram_matrix, right_vector)

    result_polynom = 0
    for ind, c in enumerate(coefficients_vector):
        result_polynom += c[0] * basis_list[ind]
    ...
    delta = np.linalg.norm(perturbed_coefficients_vector - coefficients_vector) / np.linalg.norm(perturbed_right_vector)
